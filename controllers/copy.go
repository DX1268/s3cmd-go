...
func copyFile(config *config.Config, src, dst *FileURI, ensure_directory bool) error {
	if config.Verbose {
...
func copyToLocal(config *config.Config, src, dst *FileURI, ensure_directory bool) error {
...
func copyToS3(config *config.Config, src, dst *FileURI) error {
...
func copyOnS3(config *config.Config, src, dst *FileURI) error {
	svc, err := SessionForBucket(config, dst.Bucket)
	if err != nil {
		return err
	}
