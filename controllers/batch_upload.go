...
//
func BatchUpload(config *config.Config, c *cli.Context) error {
	const (
		ACT_COPY     = iota
		ACT_REMOVE   = iota
		ACT_CHECKSUM = iota
...
//
func buildFileInfo(config *config.Config, src *FileURI, dropPrefix int, addPrefix string) (map[string]*FileObject, error) {
...
func getFileInfo(config *config.Config, srcs []*FileURI) map[FileURI]*FileObject {
...
func workerCopy(config *config.Config, wg *sync.WaitGroup, jobs <-chan Action, progress chan int64) {
	for item := range jobs {
...
func workerRemove(config *config.Config, wg *sync.WaitGroup, jobs <-chan Action, progress chan int64) {
...
func workerChecksum(config *config.Config, wg *sync.WaitGroup, jobs <-chan Action, progress chan int64) {
	for item := range jobs {
		var (
			hash string
			err  error
